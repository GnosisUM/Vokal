...
# only up to 10 speakers (to be added)
...
def waveplot(signal, sr, start_idx=0, figsize=(5, 3), color='tab:blue'):
    ...
    end_time = start_time + (len(signal) / sr)
...
def combined_waveplot(signal, sr, segments, figsize=(10, 3), tick_interval=60):
    """
    ...
    xticks = np.arange(0, (len(signal)//sr)+1, tick_interval)
...
def waveplot_perspeaker(signal, sr, segments):
    """
    ...
    for seg in segments:
        ...
        speech = signal[start:end]
        color = colors[seg['label']]
        waveplot(speech, sr, start_idx=start, color=color)
        plt.show()
